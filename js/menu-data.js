np.menuData = [{
  ...
  menuItems: [{
    ...
    shortcut: 'Ctrl+E',
    enabled: true,
    handler: () => console.log('使用Bing搜索')
  }, {
    title: '查找(F)...',
    shortcut: 'Ctrl+F',
    enabled: false,
    handler: () => console.log('查找')
  }, {
    title: '查找下一个(N)',
    shortcut: 'F3',
    enabled: false,
  ...
  menuItems: [{
    ...
    handler: () => {
      ...
      if(np.bWrap) { // 自动换行=true
        $statusBar.display(false);
        $editor.resize(false);
        $menubar.enabled(3, 0, false);   // 状态栏菜单禁用
        $menubar.enabled(1, 9, false);   // 转到菜单禁用
      } else { // 自动换行=false
        $statusBar.display(np.bShowStatusBar);
        $editor.resize(np.bShowStatusBar);
        $menubar.enabled(3, 0, true);     // 状态栏菜单启用
        $menubar.enabled(1, 9, true);     // 转到菜单启用
